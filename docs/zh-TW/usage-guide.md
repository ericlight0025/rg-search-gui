...
1. 啟動程式
2. 先停在左側 `搜尋` 頁籤，加入一個小型資料夾
3. 在搜尋文字欄位輸入一個明確關鍵字
4. 按 `開始搜尋`
5. 確認右上有檔案列表，右下可看到命中內容預覽
...
你也可以手動按左側的 `安裝 rg` 按鈕，或從上方 `設定` 選單安裝。

## 6. 設定與診斷

主畫面現在是接近 VS Code 的工作區佈局：

- 左側側欄：`搜尋` / `設定` 頁籤
- 右上：檔案列表
- 右下：內容預覽與命中切換

當你要調整搜尋行為，或確認目前偵測到的搜尋引擎狀態時，主要使用左側的 `設定` 頁籤。
...
- `開啟設定頁籤`：切到左側 `設定` 頁籤
- `診斷資訊`：切到同一頁籤，並刷新目前偵測到的引擎、執行檔路徑、版本、設定檔位置與資料夾數量
- `安裝 ripgrep (rg)`：手動觸發 Windows `winget` 安裝流程
...
設定頁籤會留在主畫面裡，因此你可以一邊調整，一邊保留目前結果畫面；程式仍會在正常流程中自動保存設定。
